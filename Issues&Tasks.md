...
This next piece of my home lab was actually pretty fun to do and fairly simple to setup. I wanted a local LLM just to be able to say I have AI running locally in my network. I hope to use this project for other projects, like pen testing in a secure, ethical environment with VMs.
...
3. Next, to actually test Ollama is working, we want to get some models to run. From what I've read on, smaller models like `llama3.2:3b`, `phi3`/`phi3:mini`, or `gemma:2b` are good for running on Raspberry Pis, since they are smaller models with smaller parameters, which I belive to correlate with less resources required to run the model. I went with the phi3 model, so the command to pull those models to your Pi is `ollama pull phi3`, or replace `phi3` with whatever model you desire. Optionally, you can test the models right now by running `ollama run phi3`, the command prompt will change to `>>>` to accept AI prompts, which then you can test the model. Crtl+C or `/exit` to exit the model.
4. Cool, sweet, we got the AI model running, but terminal is ugly tbh. Next step was to get the webUI so that we can have a nicer layout for our prompts, and to be able to access the AI model from other devices on the network. To do this, we need to install Docker with another simple command, `curl -fsSL https://get.docker.com | sh`.
...
6. To install the Open WebUI for this project, we will need to create the container using these settings below. The issue I ran into was not running it with the `--network=host` line. This line tells the container to use the same network as the host, not the containers' network. 
  ```
